...
using RabbitMQ.ExcelCreate.Shared;
using System.Data;
...
namespace FileCreateWorkerService
{
	public class Worker : BackgroundService
	{
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
			...
			var baseUrl = "https://localhost:44398/api/files";
			...
			using (var httpClient = new HttpClient())
			{
                ...
                else
                {
                    ...
                    _logger.LogError($"File (ID: {createExcelMessage.FileId}) creation failed with status code: {response.StatusCode}. Reason: {response.ReasonPhrase}");

                    // Response içeriðini yazdýr
        ...
        private DataTable GetTable(string tableName)
		{
			...
			{
				var context = scope.ServiceProvider.GetRequiredService<AdventureWorksDw2016Context>();
			...
			products.ForEach(x =>
			{
				table.Rows.Add(x.ProductKey, x.ProductAlternateKey,
					x.EnglishProductName, x.Color);
			});
